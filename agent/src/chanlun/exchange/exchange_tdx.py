...
class ExchangeTDX(Exchange):
    ...
    def _retry_failed_batches(self, client, market, failed_batches, dfs):
        """Retry failed batches using error-tolerant GBK decoding.

        pytdx's get_security_list uses strict GBK decoding which fails on
        certain stock names.  This method temporarily patches the parser's
        parseResponse to use ``errors='replace'`` so that batches with bad
        encodings are still usable (stock codes are ASCII-safe; only names
        may get garbled).
        """
        import struct
        ...
        # Save original parseResponse
        _orig_parse = GetSecurityList.parseResponse

        def _tolerant_parse(self_cmd, body_buf):
            pos = 0
            ...
            for i in range(num):
                ...
                if len(one_bytes) < 29:
                    pos += 29
                    continue
                (code, volunit, name_bytes, reversed_bytes1,
                 decimal_point, pre_close_raw, reversed_bytes2) = struct.unpack(
                    "<6sH8s4sBI4s", one_bytes
                )
                # Use errors='replace' instead of strict GBK/UTF-8
        ...
        # Temporarily patch the class method
        GetSecurityList.parseResponse = _tolerant_parse
        try:
            for batch_idx in failed_batches:
                try:
                    batch = client.get_security_list(market, batch_idx * 1000)
                    if batch:
                        dfs.append(client.to_df(batch))
                        print(f"获取股票列表批次 {market}/{batch_idx} 重试成功: {len(batch)} 条")
                except Exception as _e2:
                    print(f"获取股票列表批次 {market}/{batch_idx} 重试也失败: {_e2}")
        finally:
            # Restore original method
            GetSecurityList.parseResponse = _orig_parse
    ...
    def all_stocks(self):
        ...
        if self.connect_info is None:
            raise Exception("TDX connect_info not available, please check TDX server connectivity")
        ...
        try:
            for market in range(2):
                ...
                with client.connect(self.connect_info["ip"], self.connect_info["port"]):
                    ...
                    # Fetch security list in batches, skip batches with GBK decode errors
                    dfs = []
                    failed_batches = []
                    for i in range(int(count / 1000) + 1):
                        try:
                            batch = client.get_security_list(market, i * 1000)
                            if batch:
                                dfs.append(client.to_df(batch))
                        except TdxConnectionError:
                            raise  # let outer handler retry with new IP
                        except (UnicodeDecodeError, Exception) as _e:
                            print(f"获取股票列表批次 {market}/{i} 失败: {_e}")
                            failed_batches.append(i)
                            continue
                    # Retry failed batches with GBK error-tolerant parsing
                    if failed_batches:
                        self._retry_failed_batches(client, market, failed_batches, dfs)
                    if not dfs:
                        continue

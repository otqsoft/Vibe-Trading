...
export function Settings() {
    ...
    Promise.all([api.getLLMSettings(), api.getDataSourceSettings()])
      ...
      .catch((error) => {
        ...
        if (isAuthRequiredError(error)) {
          ...
          toast.error(`${i18n.t("settings.failedToLoadLLMSettings")}: ${message}`);
          toast.error(`${i18n.t("settings.failedToLoadDataSourceSettings")}: ${message}`);
        }
      })
      .finally(() => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`${i18n.t("settings.failedToSaveLLMSettings")}: ${error instanceof Error ? error.message : i18n.t("settings.unknownError")}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const submitDataSources = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`${i18n.t("settings.failedToSaveDataSourceSettings")}: ${error instanceof Error ? error.message : i18n.t("settings.unknownError")}`);
    } finally {
      setDataSaving(false);
    }
  };
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{i18n.t("settings.settingsDesc")}</p>
        </div>
  ...
  const keyStatus = settings.api_key_configured
    ...
    : settings.api_key_required
      ? i18n.t("settings.leaveBlankCurrentKey")
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? i18n.t("settings.oauthProviderHint", { command: selectedProvider.login_command })
  ...
  const tushareStatus = dataSettings.tushare_token_configured
    ? i18n.t("settings.configured")
    : i18n.t("settings.leaveBlankCurrentToken");
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{i18n.t("settings.changingProvidersHint")}</span>
            </label>
            ...
            <label className="grid gap-2">
              <span className={labelClass}>
                {selectedProvider?.auth_type === "oauth" ? i18n.t("settings.oauth") : i18n.t("settings.apiKey")}
              </span>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {selectedProvider?.api_key_required ? (
                  <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    {i18n.t("settings.clearSavedApiKey")}
                  </label>
                ) : null}
              </div>
            </label>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{i18n.t("settings.tushareTokenDesc")}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
                    ...
                    className="h-3.5 w-3.5 accent-primary"
                  />
                  {i18n.t("settings.clearSavedTushareToken")}
                </label>
              </div>
            </label>
